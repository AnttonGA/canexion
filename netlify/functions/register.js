...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (wkList.length > 0) {
      ...
      foundInWakyma  = true;
      // Enriquecer solo campos que el usuario dejó vacíos
      if (!email && (wkC.email || wkC.Email))
        enrichedEmail = wkC.email || wkC.Email;
      if (!postal_code && (wkC.cp || wkC.postalCode))
        enrichedCP = wkC.cp || wkC.postalCode;
    }
